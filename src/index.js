...
function extractDisclosureSearchTerms(q, ticker) {
  const stopwords = new Set([
    ...
    "risk", "risks", "company", "stock", "mine", "mining"
  ]);
...
/* ============================================================
   ✅ Symbol inference so user can ask directly without ticker box
============================================================ */
...
async function runAssistant(env, question, context) {
  const system =
    ...
    "- Users may ask directly without entering a ticker first.\n" +
    "- If DATA includes a ticker/symbol inferred from the question, use it.\n" +
    "- If no company/ticker is clear from the user question or DATA, answer generally using only available mining-related DATA.\n" +
    ...
    "- Most recent stock price and most recent financial performance data (if explicitly requested and in DATA)\n\n" +
...
/* ============================================================
   ✅ Auth helper for YouTube ingest endpoints
============================================================ */
  ...
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === "/api/ai/ask" && request.method === "POST") {
        ...
        const inferredSymbol = explicitSymbol || inferTickerFromQuestion(q);
        const limit = Math.min(
        ...
        if (inferredSymbol) {
          sql = `
            ...
            LIMIT ?
          `;
          bindings = [inferredSymbol, q, limit];
        } else {
          sql = `
            SELECT
              s.video_id, s.start, s.duration, s.text,
        ...
        const filingMatches = await getMiningDisclosureMatches(env, inferredSymbol || null, q, limit);
      ...
      if (url.pathname === "/api/assistant" && request.method === "GET") {
        return text('OK. Use POST /api/assistant with JSON body like {"question":"What risks did AEM mention?"} or {"symbol":"HYMC","question":"..."}');
      }
